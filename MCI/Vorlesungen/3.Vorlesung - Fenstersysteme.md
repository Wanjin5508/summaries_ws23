...
典型的**模态对话框**包括登录窗口、确认对话框或要求特定用户操作的弹出窗口。但在网页上的交互也可以通过其他**非模态**元素进行。例如，表单、链接、按钮和其他用户界面元素允许用户与页面互动，而不会阻止访问页面的其余内容。使用哪种交互机制取决于网页或应用程序的设计和目的。
...
0. – GUI-Fenstersystem  
    ...
    – 3) UIMS (Dialogbeschreibungssprache und Simulationskomponente) – 4) auto. gen. Werkzeuge
